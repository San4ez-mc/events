...
- [x] **Mobile** (Expo Router, apps/mobile): реальні екрани замінили
      ...
      завеликий aspect-ratio карток), вхід, пошук, «Мої події», сповіщення,
      профіль, реєстрація на подію (без помилок у консолі). **Ще не
      перевірено**: реальний телефон/симулятор, push-сповіщення на пристрої.
...
## Сесія 25.09.2026 — добивання недоробок по ТЗ

Виправлення до попередніх записів: нагадування/джоби **існують**
(`event-lifecycle.scheduler.ts`) — раніше в аудиті помилково позначались як відсутні.

Зроблено (API + web + додаток, усе на проді крім останніх двох пунктів):
- Google-вхід (API + web + Android/iOS клієнти), reset-password сторінка.
- Соціальний доказ (учасники opt-in, «друзі йдуть»), точна адреса лише після реєстрації (§10).
- Повний набір фільтрів §7 (район, бюджет, місткість, години, 18+) на web і в додатку; неповнолітнім 18+ події ховаються (birthDate).
- Галерея на сторінці події, демо-фото/відео; створення події в додатку; «Мої події».
- Автопідказка адрес (Google Places через API-проксі) у web і в додатку.
- Чат події (web + додаток), аналітика: показ/перегляд/поширення з джерелом; воронка в кабінеті організатора та на адмін-дашборді.
- Профіль на сайті (`/profile`): фото (`POST /users/me/avatar`), дані, приватність, сповіщення.
- `sitemap.xml`, `robots.txt`, скрипт бекапів `infrastructure/deploy/backup.sh` + інструкція відновлення.
- Повний аудит по розділах ТЗ: `docs/SPEC_AUDIT.md`.

Очікує: розгортання останніх комітів (профіль/аватар, sitemap, адмін-воронка), перебудова APK,
SMTP-доступи (лист для reset-password), встановлення cron бекапів на VPS.
